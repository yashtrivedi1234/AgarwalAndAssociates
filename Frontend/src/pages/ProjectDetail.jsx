    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-6 py-20">
        <div className="flex justify-between items-end mb-12">
          <div>
            <span className="inline-block px-4 py-2 bg-red-100 text-red-600 text-xs font-bold uppercase tracking-widest rounded mb-4">
              More Work
            </span>
            <h2 className="text-4xl lg:text-5xl font-black text-gray-900">Similar Projects</h2>
          </div>
          <Link 
            to="/projects"
            className="hidden md:flex items-center gap-3 px-8 py-4 bg-gray-900 hover:bg-red-600 text-white font-bold uppercase tracking-wide rounded-lg transition-all duration-300"
          >
            View All Projects
            <ArrowRight className="w-5 h-5" />
          </Link>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {relatedProjects.length > 0 ? relatedProjects.map((proj, index) => (
            <Link
              key={proj.id}
              to={`/project/${proj.slug}`}
              className="group block"
            >
              <div className="border-2 border-gray-200 hover:border-red-600 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
                <div className="relative aspect-[16/10] overflow-hidden">
                  <img 
                    src={proj.image || `https://source.unsplash.com/random/800x600?sig=${index + 1}`} 
                    alt={proj.title}
                    className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                  />
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-3">
                    <ExternalLink className="w-8 h-8 text-red-500" />
                    <span className="text-white font-bold uppercase tracking-wider text-sm">View Project</span>
                  </div>
                </div>
                <div className="p-6">
                  <span className="inline-block px-3 py-1 bg-gray-100 text-red-600 text-xs font-bold uppercase tracking-wider rounded mb-3">
                    {proj.category}
                  </span>
            ...
            // Dummy related projects if no data
            [1, 2, 3].map((num) => (
              <div key={num} className="group block">
                <div className="border-2 border-gray-200 hover:border-red-600 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2">
                  <div className="relative aspect-[16/10] overflow-hidden">
                    <img 
                      src={`https://source.unsplash.com/random/800x600?sig=sim${num}`} 
                      alt={`Related Project ${num}`}
                      className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                    />
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-3">
                      <ExternalLink className="w-8 h-8 text-red-500" />
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 group-hover:text-red-600 transition-colors duration-300 mb-3">
                      Modern Architecture Project {num}
                    </h3>
                    <div className="flex items-center gap-2 text-gray-600 text-sm">
                      <MapPin className="w-4 h-4 text-red-500" />
        ...
        <Link 
          to="/projects"
          className="md:hidden flex items-center justify-center gap-3 mt-8 px-8 py-4 bg-gray-900 hover:bg-red-600 text-white font-bold uppercase tracking-wide rounded-lg transition-all duration-300 w-full"
        >
          View All Projects
          <ArrowRight className="w-5 h-5" />
        </Link>
